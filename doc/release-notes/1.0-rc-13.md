...
* **Faster script compilation** ([#1168](https://github.com/gradle/kotlin-dsl/issues/1168), [#817](https://github.com/gradle/kotlin-dsl/issues/817), [#TODO](https://github.com/gradle/kotlin-dsl/issues/TODO))
    ...
    In this release we replaced the usage of the Kotlin compiler by generating the bytecode directly instead. The generated accessors are lean and repetitive, no need for a full-blown compiler to produce their bytecode.
    ...
    On a small Android application, here the Android Sunflower sample application, the feedback loop on API change in `buildSrc` want from ~7s down to ~4.5s. That's a 40% improvement too.
